...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**₵𝕐𝔹𝔼℞☾฿𝕌₢𝕂☽『℧𝕊𝔼ℝß𝕆𝕋』"):
            buttons = [
                (Button.url(f"{ALIVE_NAME}", f"tg://openmessage?user_id={USERID}"),),
                (
                    Button.inline("Stats", data="stats"),
                    Button.url("Repo", "https://github.com/INVISIBLE-CYBER-BUCK/CYBERBUCKUSERBOT"),
                ),
            ]
            ALIVE_PIC = gvarstatus("ALIVE_PIC")
            if ALIVE_PIC is None:
                I_IMG = "https://telegra.ph/file/6359f460ffdddd84a9d06.jpg"
            else:
        ...
        if query.startswith("**⚜ ₵𝕐𝔹𝔼℞☾฿𝕌₢𝕂☽『℧𝕊𝔼ℝß𝕆𝕋』"):
            ...
            ALIVE_PIC = gvarstatus("ALIVE_PIC")
            if ALIVE_PIC is None:
                IMG = "https://telegra.ph/file/6359f460ffdddd84a9d06.jpg"
            else:
            ...
            result = builder.article(
                title="Repository",
                text=f"**⚜ Legendary Af ₵𝕐𝔹𝔼℞☾฿𝕌₢𝕂☽『℧𝕊𝔼ℝß𝕆𝕋』 ⚜**",
                buttons=[
                    [Button.url("♥️ Tutorial ♥", "https://youtu.be/CH_KO1wim2o")],
                    [Button.url("📍 𝚁𝚎𝚙𝚘 📍", "https://github.com/INVISIBLE-CYBER-BUCK/CYBERBUCKUSERBOT")],
                    [
                        Button.url(
                            "💞 Deploy 💞",
            ...
            builder = event.builder
            troll = os.path.join("./Cyberbot", "troll.txt")
            try:
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./Cyberbot", "secrets.txt")
            try:
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./Cyberbot", "hide.txt")
            try:
            ...
            if oso is None:
                help_pic = "https://telegra.ph/file/6359f460ffdddd84a9d06.jpg"
            else:
            ...
            if oso == "OFF":
                result = builder.article(
                    title="© CyberBot Help",
                    description="Help menu for CyberBot",
                    text=_result[0],
                    buttons=_result[1],
                    link_preview=False,
                )
            ...
            PM_IMG = (
                gvarstatus("PM_IMG")
                or "https://telegra.ph/file/6359f460ffdddd84a9d06.jpg"
            )
            if PM_IMG == "OFF":
                LEGEND_IMG = None
            else:
            ...
            ALV_PIC = "https://telegra.ph/file/6359f460ffdddd84a9d06.jpg"
            markup = event.client.build_reply_markup(buttons)
            photo = types.InputWebDocument(
            ...
            result = types.InputBotInlineResult(
                id=str(uuid4()),
                type="photo",
                title=f"₵𝕐𝔹𝔼℞☾฿𝕌₢𝕂☽『℧𝕊𝔼ℝß𝕆𝕋』",
                description=f"₵𝕐𝔹𝔼℞☾฿𝕌₢𝕂☽『℧𝕊𝔼ℝß𝕆𝕋』\nhttps://t.me/invisiblesecuritycyberbuckchat",
                url="https://github.com/INVISIBLE-CYBER-BUCK/CYBERBUCKUSERBOT",
                thumb=photo,
                content=photo,
                send_message=types.InputBotInlineMessageMediaAuto(
                    reply_markup=markup, message=text, entities=msg_entities
...
async def on_plug_in_callback_query_handler(event):
    ...
    await event.edit(
        f"📜 Menu Provider Has Been Closed\n\n🔰 Bot Of : {mention}\n\n             [©️₵𝕐𝔹𝔼℞☾฿𝕌₢𝕂☽『℧𝕊𝔼ℝß𝕆𝕋』](https://t.me/invisiblesecuritycyberbuckchat)",
        buttons=buttons,
        link_preview=False,
    )
