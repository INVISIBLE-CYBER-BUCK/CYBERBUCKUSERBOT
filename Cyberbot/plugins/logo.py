...
async def very(event):
    ...
    LOGO_BACKGROUND = (
        ...
        or "https://raw.githubusercontent.com/ITS-Cyberbot/RESOURCES/master/background/black.jpg"
    )
    LOGO_FONT = (
        gvarstatus("LOGO_FONT")
        or "https://github.com/ITS-Cyberbot/RESOURCES/blob/master/fonts/VampireWars.ttf?raw=true"
    )
...
async def lg1(event):
    ...
    if event.reply_to_msg_id:
        ...
        async for i in event.client.iter_messages(
            "@Cyberbot_Logos", filter=InputMessagesFilterPhotos
        ):
            PICS_STR.append(i)
...
async def bad(event):
    ...
    source = requests.get(
        "https://github.com/ITS-Cyberbot/RESOURCES/tree/master/background"
    )
    ...
    if input_str not in bg_name:
        ...
        string = f"https://raw.githubusercontent.com/ITS-Cyberbot/RESOURCES/master/background/{input_str}.jpg"
...
async def pussy(event):
    ...
    if cmd == "":
        source = requests.get(
            "https://github.com/ITS-Cyberbot/RESOURCES/tree/master/fonts"
        )
        soup = BeautifulSoup(source.text, features="html.parser")
        links = soup.find_all("a", class_="js-navigation-open Link--primary")
        logo_font = []
        ...
        if input_str not in logo_font:
            ...
            string = f"https://github.com/ITS-Cyberbot/RESOURCES/blob/master/fonts/{input_str}.ttf?raw=true"
