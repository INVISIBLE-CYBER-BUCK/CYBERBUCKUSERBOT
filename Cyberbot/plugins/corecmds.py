...
def plug_checker(plugin):
    plug_path = f"./Cyberbot/plugins/{plugin}.py"
    if not os.path.exists(plug_path):
        plug_path = f"./xtraplugins/{plugin}.py"
    if not os.path.exists(plug_path):
        plug_path = f"./Cyberbot/assistant/{plugin}.py"
    return plug_path
...
@legend.legend_cmd(
    ...
    info={
        ...
        "description": "Reply to any external plugin(supported by Cyberbot) to install it in your bot.",
        "usage": "{tr}install",
    },
)
        ...
        try:
            downloaded_file_name = (
                await event.client.download_media(  # pylint:disable=E0602
                    ...
                    "./Cyberbot/plugins/",  # pylint:disable=E0602
                )
            )
