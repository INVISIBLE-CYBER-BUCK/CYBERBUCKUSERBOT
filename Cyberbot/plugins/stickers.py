...
def pack_name(userid, pack, is_anim, is_vid):
    if gvarstatus("CUSTOM_STICKER_SETNAME") is not None:
        ...
        return f"Cyberbot_{userid}_{pack}_anim"
    elif is_vid:
        return f"Cyberbot_{userid}_{pack}_vid"
    else:
        return f"Cyberbot_{userid}_{pack}"
    ...
    for message in reqd_sticker_set.documents:
        ...
        if photo:
            ...
            if (
                ...
                async with args.client.conversation("@Stickers") as conv:
                    pack, Cyberbotname = await newpacksticker(
                        legendevent,
                        conv,
                        cmd,
                        args,
                ...
                async with args.client.conversation("@Stickers") as conv:
                    pack, Cyberbotname = await add_to_pack(
                        legendevent,
                        conv,
                        args,
                        packname,
            ...
            if Cyberbotname is None:
                return
            if Cyberbotname not in blablapacks:
                blablapacks.append(Cyberbotname)
