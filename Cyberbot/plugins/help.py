...
async def grpinfo():
    outstr = "**Plugins in Cyberbot are:**\n\n"
...
async def cmdlist():
    outstr = "**Total list of Commands in your Cyberbot are :**\n\n"
...
@legend.legend_cmd(
    ...
    info={
        "header": "To get guide for Cyberbot.",
...
async def _(event):
    ...
    if not input_str:
        ...
        for cmd in cmds:
            outstr += f"  - `{cmdprefix}{cmd}`\n"
        outstr += f"**👨‍💻  Usage : ** `{cmdprefix}help -l <command name>`"
    await eor(event, outstr, aslink=True, linktext="Total Commands of Cyberbot are :")
