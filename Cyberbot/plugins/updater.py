...
from Cyberbot import HEROKU_APP, UPSTREAM_REPO_URL, legend
...
lb_info = "https://raw.githubusercontent.com/ITS-Cyberbot/Cyberbot/master/LegendBoy-info.json"


async def ld_info(lb_info):
    infos = requests.get(lb_info).json()
    _version = infos["Cyberbot-INFO"]["version"]
    _release = infos["Cyberbot-INFO"]["release-date"]
    _branch = infos["Cyberbot-INFO"]["branch"]
    _author = infos["Cyberbot-INFO"]["author"]
    _auturl = infos["Cyberbot-INFO"]["author-url"]
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    ...
    if heroku_app is None:
        await event.edit(
            f"{txt}\n" "`Invalid Heroku credentials for deploying Cyberbot dyno.`"
        )
        return repo.__del__()
    LEGEND = await event.edit(
...
async def upstream(event):
    ...
    if ENV and (API_KEY is None or APP_NAME is None):
        return await eor(
            event,
            "`👨‍💻 Set the required vars first to Update the [Lêɠêɳ̃dẞø†](https://t.me/Cyberbot_AI/292)`",
        )
    try:
        txt = (
        ...
        if conf is None:
            return await event.edit(
                f"`Unfortunately, the directory {error} does not seem to be a git repository.\nBut we can fix that by force updating the Cyberbot using .update now.`"
            )
        repo = Repo.init()
    ...
    if force_update:
        await event.edit(
            "`Force-Syncing to latest stable Cyberbot code, please wait...`"
        )
    if conf == "now":
        await event.edit("`Updating Cyberbot, please wait....`")
...
async def upstream(event):
    event = await eor(event, "`Pulling the Cyberbot repo wait a sec ....`")
    off_repo = "https://github.com/LEGEND-AI/Cyberbot"
    os.chdir("/app")
    try:
        txt = (
            "`Oops.. Updater cannot continue due to "
    ...
    ups_rem = repo.remote("upstream")
    ups_rem.fetch(ac_br)
    await event.edit("`Deploying Cyberbot, please wait....`")
...
async def variable(var):
    ...
    heroku_var["UPSTREAM_REPO"] = "https://github.com/ITS-Cyberbot/LEGENDUSERBOT"
