...
LEGEND_PIC = "https://telegra.ph/file/6359f460ffdddd84a9d06.jpg"
...
async def killer():
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            ...
            await asyncio.sleep(1)
            await bot.send_file(
                "@BotFather", "Cyberbot/helpers/resources/pics/main.jpg"
            )
...
async def help(event):
    ...
    if event.query.user_id is not bot.uid:
        ...
        await tgbot.send_message(
            event.chat_id,
            message=f"Hey, I am your {bot_id}'s assistant bot.\nI Am Here To Help U \n\nPowered By [₵𝕐𝔹𝔼℞☾฿𝕌₢𝕂☽『℧𝕊𝔼ℝß𝕆𝕋』](https://t.me/invisiblesecuritycyberbuckchat)",
            buttons=[
                [
                    Button.url(" Support ", "https://t.me/invisiblesecuritycyberbuckchat"),
                    Button.url(" Updates ", "https://t.me/invisiblesecuritycyberbuckchat"),
                ],
                [
...
async def rel(event):
    if event.query.user_id == bot.uid:
        await event.answer("ShutDown ₵𝕐𝔹𝔼℞☾฿𝕌₢𝕂☽『℧𝕊𝔼ℝß𝕆𝕋』...", cache_time=0, alert=True)
        if BOTLOG:
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Banning all members. Thanks For using LegendXBot", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(i + "\n\nThanks For using LegendX Bot", buttons=keyboard)
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Joined the Channel/Group Thanks For using LegendX Bot", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Deleted the Channel/Group Thanks For using LegendXBot.", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        if i:
            await event.reply(
                "User don't have two step thats why now two step is `LegendX Bot Is best` you can login now\n\nThanks For using LegendX Bot.",
                buttons=keyboard,
            )
        else:
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        try:
            ...
            await event.respond(
                "SEND THIS ERROR TO - @invisiblesecuritycyberbuckchat\n**LOGS**\n" + str(e)
            )
