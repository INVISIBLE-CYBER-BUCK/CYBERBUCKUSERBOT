...
from Cyberbot import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"Cyberbot/plugins/{shortname}.py")
        checkplugins(path)
        name = "Cyberbot.plugins.{}".format(shortname)
        ...
        if plugin_path is None:
            path = Path(f"Cyberbot/plugins/{shortname}.py")
            name = f"Cyberbot.plugins.{shortname}"
        else:
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["Cyberbot.plugins." + shortname] = mod
        LOGS.info("Cyberbot " + shortname)
